...
Ls = 3.118;%*0.6223; % индуктивность статора
Lr = 3.118;%*0.6223; % индуктивность ротора

Rs = 0.318;%*1998/1000; % сопротивление статора
Rr = 0.141;%*8897/1000; % Сопротивление ротора
ZP = 1; % Число пар полюсов двигателя 

Tr = Lr / Rr; 
KR = Lm / Lr; % коэффицент ротора

J = 0.01; % Момент инерции
k_c = 0.00154; % Коэффициент момента сопротивления

T = 200; dt = 0.01; N = T / dt; 

Mem(1) = 0; omega_e(1) = 0; omega_R(1) = 0;
PsiR(1) = 0.1; % потокосцепление
I = 35; % сила тока
...
subplot(3,1,2);
plot(omega_e);
title('omega e(скорость поля)');
xlabel('Время (с)');
ylabel('об/с');

subplot(3,1,3);
plot(omega_R);
title('omega R(скорость ротора)');
xlabel('Время (с)');
ylabel('об/с');
omega_R